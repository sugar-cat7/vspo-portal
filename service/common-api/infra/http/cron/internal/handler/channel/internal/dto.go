...
// ChannelsResponse converts model.Channels to []api.ChannelResponse
func ChannelsResponse(c model.Channels) []api.ChannelResponse {
	creators := make([]api.ChannelResponse, len(c))
	for i, creator := range c {
		creators[i] = ChannelResponse(creator)
	}
	return creators
}
