...
export class DiscordServerRepository implements IDiscordServerRepository {
  ...
  async list(query: ListQuery): Promise<Result<DiscordServers, AppError>> {
    return withTracer("DiscordServerRepository", "list", async (span) => {
  ...
  async count(query: ListQuery): Promise<Result<number, AppError>> {
    return withTracer("DiscordServerRepository", "count", async (span) => {
      const discordServerResult = await wrap(
        this.db
          .select({ count: count() })
          .from(discordServerTable)
  ...
  > {
    return withTracer("DiscordServerRepository", "get", async (span) => {
      const discordServerResult = await wrap(
        this.db
          .select()
          .from(discordServerTable)
  ...
  > {
    return withTracer("DiscordServerRepository", "exists", async (span) => {
      const discordServerResult = await wrap(
        this.db
          .select()
          .from(discordServerTable)
